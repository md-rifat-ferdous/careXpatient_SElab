...
async function getAuthenticatedDoctorId() {
...
function parseTimesFromShift(shift) {
...
function parseSlotDescription(description) {
...

{
  try {
    ...
    if (targetClinicMapping) {
      ...
      if (times) {
        const fmtH = (m) => Math.floor(m / 60).toString().padStart(2, '0');
        const fmtM = (m) => (m % 60).toString().padStart(2, '0');
...
// ─── Rollback Override ────────────────────────────────────────────────────────
export async function rollbackOverride(id) {
  try {
