...
interface LabOrder {
  ...
  createdAt: string;
  lab?: { name: string; address?: string };
  tests: Array<{ labTest: { name: string; price: number; sampleType?: string } }>;
  result?: { resultSummary?: string; fileUrl?: string };
}
...
function OrderCard({ order, onViewDetails }: { order: LabOrder; onViewDetails: (o: LabOrder) => void }) {
...
function OrderDetailModal({ order, onClose }: { order: LabOrder; onClose: () => void }) {
  const totalCost = order.tests.reduce((sum, t) => sum + t.labTest.price, 0);
