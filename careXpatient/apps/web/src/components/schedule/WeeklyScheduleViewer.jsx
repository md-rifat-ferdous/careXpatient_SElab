...
import {
  format, startOfWeek, addDays, isSameDay, isToday } from
'date-fns';
...
function parseDaysFromShift(shift) {
  if (!shift) return [];
  const dayStr = shift.split('|')[0]?.trim() ?? '';
  return dayStr.split(',').map((d) => d.trim());
}

function getOverrideForDay(modifications, clinicId, date) {
...
export function WeeklyScheduleViewer({ clinics, modifications, weekStart }) {
