...
// Similar API endpoints for mobile
...
app.post('/api/requests/:id/decline', async (req, res) => {
  ...
  try {
    console.log(`Mobile: Appointment ${id} cancelled. Reason: ${reason}`);
    await pool.query("UPDATE appointment_requests SET status = 'declined' WHERE id = $1", [id]);
...
app.listen(port, () => {
  console.log(`Mobile backend listening at http://localhost:${port}`);
});
