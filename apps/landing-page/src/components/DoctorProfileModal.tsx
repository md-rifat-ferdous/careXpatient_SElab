...
export default function DoctorProfileModal({ doctor, onClose, onBookNow }: DoctorProfileModalProps) {
  ...
  return (
    <>
      ...
      <div
        onClick={onClose}
        className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-[fadeIn_0.2s_ease]"
      >
        {/* Modal */}
        <div
          onClick={(e) => e.stopPropagation()}
          className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-[slideUp_0.25s_ease]"
        >
          {/* Header Banner + Avatar */}
          <div className="relative">
            {/* Teal banner */}
            <div className="h-28 bg-gradient-to-r from-teal-500 to-teal-400 rounded-t-2xl overflow-hidden">
              <div className="absolute inset-0 opacity-20"
                style={{ backgroundImage: 'url("data:image/svg+xml,%3Csvg width=\'40\' height=\'40\' viewBox=\'0 0 40 40\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cg fill=\'%23fff\' fill-opacity=\'0.4\'%3E%3Ccircle cx=\'20\' cy=\'20\' r=\'3\'/%3E%3C/g%3E%3C/svg%3E")' }}>
              </div>
            </div>
          ...
          {/* Spacer so content doesn't sit under the avatar */}
          <div className="h-12" />
          ...
          <div className="px-6 pb-6 space-y-6">
            ...
            <div>
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-xl font-black text-slate-800">{doctor.name}</h2>
                  <p className="text-sm font-semibold text-teal-600 mt-0.5">{doctor.specialty}</p>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                { label: 'Patients', value: `${doctor.reviewCount * 2}+`, icon: '👥' },
            ...
            {/* Specializations */}
            <div>
              <h3 className="text-sm font-bold text-slate-700 mb-2">Specializations</h3>
              <div className="flex flex-wrap gap-2">
                {[doctor.specialty, 'Telemedicine', 'Preventive Care', 'Emergency'].map((tag) => (
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold text-slate-700">Patient Reviews</h3>
                <span className="text-xs text-teal-600 font-semibold cursor-pointer hover:underline">See all</span>
              </div>
          ...
          <div className="sticky bottom-0 bg-white border-t border-slate-100 px-6 py-4 flex gap-3 rounded-b-2xl">
            ...
            <button
              onClick={handleBook}
              className="flex-1 py-3 bg-teal-500 text-white font-bold rounded-xl text-sm shadow-lg shadow-teal-500/20 hover:bg-teal-600 active:scale-95 transition-all"
            >
              Book Appointment
            </button>
          </div>
