...
export function joinWaitingRoom(appointmentId: string, patientName: string, token: string): void {
  const s = getSocket(token);
  s.emit('waiting:join', { appointmentId, patientName });
}

export function leaveWaitingRoom(appointmentId: string, token: string): void {
  const s = getSocket(token);
