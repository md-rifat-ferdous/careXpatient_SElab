...
import { ChevronLeft, ChevronRight, Plus, CalendarDays, Building2, Plane, Coffee, RotateCcw, Clock } from 'lucide-react';
...
import {
  ...
  applyHoliday,
  applyLeave,
} from '@/server/doctorSchedule/actions/scheduleActions';

type Tab = 'weekly' | 'overrides';

type Props = {
  initialClinics: ClinicData[];
  initialModifications: ModificationData[];
};
      ...
      <div className="min-h-screen bg-gray-50/50">
        ...
        <div className="max-w-6xl mx-auto px-6 py-6">
          ...
          <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit mb-6">
            {(['weekly', 'overrides'] as Tab[]).map((tab) => (
              <button
                key={tab}
...
function SubmitBtn({ onClick, loading, label, color = 'bg-blue-600 hover:bg-blue-700' }: { onClick: () => void; loading: boolean; label: string; color?: string }) {
  return (
    <button
      onClick={onClick}
      disabled={loading}
