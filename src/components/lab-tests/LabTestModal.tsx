...
interface LabTestModalProps {
  ...
  isInCart: boolean;
  onAddToCart: (test: any, selectedLab: string, price: number) => void;
}

export function LabTestModal({ isOpen, onClose, test, isInCart, onAddToCart }: LabTestModalProps) {
  ...
  const getLabPrice = useCallback((labName: string, basePrice: number) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 sm:px-6">
      ...
      <div className="relative bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto z-10 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="bg-gradient-to-r from-primary to-teal-500 p-8 pb-6 rounded-t-[2rem] relative">
          ...
          <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold mb-3 ${test.tagColor}`}>
            {test.tag}
          </span>
